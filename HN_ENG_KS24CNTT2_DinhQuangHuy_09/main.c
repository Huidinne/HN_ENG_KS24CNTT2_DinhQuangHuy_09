...
void add_employee(EmployeeManager* book) {
    ...
    new_node->employee.id = book->next_id++;
    if (book->current_head == NULL) {
        book->current_head = new_node;
    } else {
        SingleNode* temp = book->current_head;
